...
exports.updateProfile = async (req, res) => {
  try {
    ...
    const allowedFields = ['name', 'phone', 'dob', 'gender', 'course', 'semester', 'yearOfStudy', 'bio', 'avatar'];
...
exports.getSavedResources = async (req, res) => {
  try {
    const user = await User.findById(req.user.id).populate({
      path: 'savedResources',
      populate: { path: 'uploadedBy', select: 'name email role avatar' }
    });
  ...
  try {
    ...
    if (isSelf) {
      user = await User.findById(req.params.id)
        .select('name avatar bio role joined course semester yearOfStudy totalUploads totalDownloads totalLikes totalDislikes documentLikes documentDislikes avgRating ratingCount profileVisits');
    } else {
...
exports.interactWithUser = async (req, res) => {
  try {
    ...
    if (rating !== undefined && rating > 0) {
      await notificationController.createNotification(
        targetUserId,
        `rated your profile ${rating} stars!`,
        'info',
        req.user.id,
        `/profile/${req.user.id}`
      );
...
exports.deleteUser = async (req, res) => {
  try {
    ...
    await AuditLog.create({
      ...
      details: `${req.user.role} permanently deleted user account ${user.email} and all associated data.`
    });
