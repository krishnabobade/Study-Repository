...
import { BookOpen, Mail, Lock, User, GraduationCap, ArrowRight, Eye, EyeOff, Check, X, Camera, Phone, Calendar, Building, FileText, ChevronLeft, ChevronRight } from 'lucide-react'
...
export default function Register() {
  ...
  return (
    <div className="min-h-screen bg-surface flex items-center justify-center p-4 relative overflow-hidden py-12">
      <div className="absolute top-0 right-0 w-96 h-96 bg-ink-600/20 rounded-full blur-3xl translate-x-1/2 -translate-y-1/2 pointer-events-none" />
      <div className="absolute bottom-0 left-0 w-96 h-96 bg-ink-800/15 rounded-full blur-3xl -translate-x-1/2 translate-y-1/2 pointer-events-none" />
      ...
      <motion.div
        ...
        <div className="glass rounded-3xl p-8 shadow-2xl overflow-y-auto max-h-[85vh] custom-scrollbar">
          ...
          <div className="flex gap-2 mb-8">
            {[1, 2, 3].map(i => (
              <div key={i} className={`flex-1 h-1.5 rounded-full transition-colors duration-300 ${step >= i ? 'bg-ink-400' : 'bg-white/10'}`} />
            ))}
          </div>
            ...
            <AnimatePresence mode='wait'>
              {step === 1 && (
                <motion.div key="step1" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} className="space-y-4">
                  ...
                  <div className="relative">
                    <Mail size={16} className={`absolute left-4 top-1/2 -translate-y-1/2 ${form.email && !form.email.toLowerCase().endsWith('@mitwpu.edu.in') ? 'text-red-400' : 'text-white/30'}`} />
                  ...
                  <AnimatePresence>
                    {(isFocused || form.password.length > 0) && (
                      <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }}
                        className="bg-white/[0.02] border border-white/5 rounded-xl p-3 grid grid-cols-2 gap-y-2 gap-x-1 text-[11px] sm:text-xs">
                        {passwordCriteria.map(c => (
                          <div key={c.id} className={`flex items-center gap-1.5 transition-colors duration-300 ${c.valid ? 'text-green-400' : 'text-white/40'}`}>
                ...
                <motion.div key="step2" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} className="space-y-4">
                  ...
                  <div className="flex items-center gap-4 mb-2">
                    <div className="relative group shrink-0">
                      <div className="w-16 h-16 rounded-2xl bg-ink-700 flex items-center justify-center overflow-hidden ring-2 ring-border group-hover:ring-ink-500 transition-all">
                        {avatarPreview ? <img src={avatarPreview} alt="avatar" className="w-full h-full object-cover" /> : <User size={24} className="text-white/30" />}
                      </div>
                    ...
                    <div>
                      <p className="text-sm font-medium text-white shadow-sm">Profile Picture</p>
                      <p className="text-xs text-white/40">Optional. Max 2MB.</p>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="relative">
                      ...
                      <input type="date" value={form.dob} onChange={set('dob')} className="input pl-11 text-white/70" />
                    </div>
              ...
              {step === 3 && (
                <motion.div key="step3" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="relative">
                      <GraduationCap size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-white/30" />
