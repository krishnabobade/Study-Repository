...
import SkeletonBase, { SkeletonAvatar, SkeletonTitle, SkeletonText, SkeletonCard } from '../components/shared/Skeleton'
      ...
      <div className="max-w-4xl mx-auto p-4 md:p-8 space-y-6">
        <div className="card p-6 md:p-8 relative overflow-hidden">
          <div className="flex flex-col md:flex-row items-center md:items-start gap-6">
            <SkeletonAvatar size={100} className="rounded-[32px]" />
            <div className="flex-1 space-y-4 w-full">
              <div className="flex flex-col md:flex-row md:items-center gap-3 justify-center md:justify-start">
                <SkeletonTitle width="40%" className="h-9" />
                <SkeletonBase className="h-6 w-20 rounded-xl opacity-40" />
              </div>
              <SkeletonText lines={2} className="opacity-40" />
              <div className="flex flex-wrap justify-center md:justify-start gap-3">
                <SkeletonBase className="h-10 w-32 rounded-xl opacity-30" />
                <SkeletonBase className="h-10 w-40 rounded-xl opacity-30" />
              </div>
            </div>
          </div>
          <div className="grid grid-cols-2 xs:grid-cols-3 lg:grid-cols-5 gap-3 mt-8 pt-8 border-t border-border">
            {[...Array(5)].map((_, i) => (
              <div key={i} className="bg-panel/50 border border-border rounded-2xl p-5 flex flex-col items-center justify-center">
                ...
                <SkeletonBase className="h-7 w-12 mb-2 opacity-60" />
                <SkeletonBase className="h-2 w-16 opacity-30" />
              </div>
            ))}
          </div>
        </div>
  ...
  return (
    <div className="max-w-4xl mx-auto p-4 md:p-8 space-y-6">
      ...
      >
        ...
        <div className="flex flex-col md:flex-row items-center md:items-start gap-6 text-center md:text-left relative z-10">
          ...
          <div className="flex-1">
            <div className="flex flex-col md:flex-row md:items-center gap-3 mb-3 justify-center md:justify-start">
              <h1 className="text-[28px] leading-none font-display font-bold text-text-main tracking-tight">{censorText(profile.name || '')}</h1>
            ...
            <p className="text-[15px] leading-relaxed text-text-muted mb-6 max-w-2xl mx-auto md:mx-0 font-medium">
              {profile.bio ? censorText(profile.bio) : "This user hasn't added a bio yet."}
            </p>
            ...
            <div className="flex flex-wrap items-center justify-center md:justify-start gap-3">
              ...
              {(profile.course || profile.semester) && (
                <div className="flex items-center gap-2 text-sm font-medium text-text-muted bg-panel border border-border py-2 px-3.5 rounded-xl shadow-inner">
                  ...
                  {profile.course ? censorText(profile.course) : ''} {profile.semester ? `• Semester ${profile.semester}` : ''}
                </div>
              )}
            </div>
        ...
        <div className="grid grid-cols-2 xs:grid-cols-3 lg:grid-cols-5 gap-3 md:gap-4 mt-8 pt-8 relative z-10 border-t border-white/[0.08]">
              ...
              <Link key={item._id} to={`/resources/${item._id}`} className="card p-4 hover:border-ink-500/30 transition-all duration-300 group hover:-translate-y-1 shadow-lg hover:shadow-ink-500/10">
                <div className="flex items-start gap-3">
                  ...
                  <div className="min-w-0 flex-1 pr-2">
                    <h3 className="text-[15px] font-semibold text-text-main/90 truncate group-hover:text-ink-400 transition-colors leading-tight mb-1">{censorText(item.title || '')}</h3>
                    <div className="flex items-center gap-1.5 text-[12px] text-text-muted font-medium">
                      <span className="truncate">{censorText(item.subject || '')}</span>
