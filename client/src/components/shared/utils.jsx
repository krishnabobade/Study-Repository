...
// Force Native File Download via Blob to bypass Cloudinary Raw constraints & Mobile Popup Blockers
export async function forceDownloadBlob(fileUrl, resourceTitle, fileType) {
  if (!fileUrl) return false;
  try {
    const safeTitle = (resourceTitle || 'document').replace(/[^a-zA-Z0-9-_ \(\)]/g, '_');
    const extMatch = fileUrl.match(/\.([a-zA-Z0-9]+)(?:[\?#]|$)/);
    let ext = extMatch ? extMatch[1] : '';
    if (!ext) {
      if (fileType === 'pdf') ext = 'pdf';
      else if (fileType === 'doc') ext = 'docx';
      else if (fileType === 'ppt') ext = 'pptx';
    ...
    // Fallback: If Blob fails (e.g. CORS or massive file), open in new tab directly
